...
def _latest_summary_url(script_dir: Path) -> Optional[str]:
    """
    Finds either *_augmentation_summary.txt (baseline) OR
    augmentation_newPipeline_results_*.txt (enhanced).
    """
    results = script_dir / "augmentation_results"
    if not results.exists():
        return None
    candidates = []
    candidates += list(results.glob("*_augmentation_summary.txt"))
    candidates += list(results.glob("augmentation_newPipeline_results_*.txt"))
    if not candidates:
        return None
...
def run_baseline(req: BaselineRequest):
    ...
    root_dir = Path(req.ROOT_DATASET_DIR).resolve()
    out_dir = Path(req.MAIN_OUTPUT_DIR).resolve()
    if not script.exists(): raise HTTPException(500, f"Script not found: {script}")
    if not root_dir.exists(): raise HTTPException(400, f"ROOT_DATASET_DIR not found: {root_dir}")
...
def run_enhanced(req: EnhancedRequest):
    ...
    root_dir = Path(req.ROOT_DATASET_DIR).resolve()
    out_dir = Path(req.AUGMENTED_OUTPUT_DIR).resolve()
    if not script.exists(): raise HTTPException(500, f"Script not found: {script}")
    if not root_dir.exists(): raise HTTPException(400, f"ROOT_DATASET_DIR not found: {root_dir}")
    out_dir.parent.mkdir(parents=True, exist_ok=True)
